...
func NewCommand() *cobra.Command {
	...
	runtime/kubernetes/etcd/kuberentes-cni/wdrip
	from: wdrip/${NAMESPACE}/${VERSION}/wdrip/amd64/linux/files
	to:   
`,
	...
	build.Flags().StringVar(&flags.Bucket, "bucket", "host-wdrip", "default bucket prefix. real bucket name would be ${prefix}-${region}, eg. host-wdrip-cn-hangzhou")
	...
	build.Flags().StringVar(&flags.Project, "project", "wdrip", "project type default wdrip")
	...
	build.Flags().StringVar(&flags.WdripVersion, "wdrip-version", "", "wdrip version 0.1.1")
...
func InitBuild(flag *flagpole) error {
	...
	// source: wdrip/kubernetes/amd64/linux/bin/{kubelet,kubectl,kubeadm}

	ctx := NewBuildContext(
		func(bctx *BuildContext) {
			bctx.Ctx = flag

			bctx.TempDir = fmt.Sprintf("%s/.wdrip/cache", os.Getenv("HOME"))
		},
	)
	return ctx.Build()
}
...
func (b *BuildContext) Build() error {
	...
	if b.Ctx.DryRun {
		...
		klog.Infof("http://%s.oss.aliyuncs.com"+
			"/wdrip/default/public/etcd/v3.4.3/amd64/centos/bin/etcd-v3.4.3.tar.gz", bucket)
	}
	for _, region := range b.Ctx.Regions {
		err := EnsureBucket(region, b.Ctx.Bucket)
		if err != nil {
			return err
		}
	}
...
func NewWdrip(b *BuildContext) *file.Path {
	return &file.Path{
		Project:   b.Ctx.Project,
		Namespace: b.Ctx.NamespaceFrom,
		CType:     b.Ctx.CloudType,
		Pkg:       "wdrip",
		Version:   b.Ctx.WdripVersion,
		Arch:      b.Ctx.Arch,
		OS:        b.Ctx.SourceOS,
		Ftype:     file.FILE_BINARY,
	}
...
func BuildWdrip(b *BuildContext, m *build) error { return doBuildWdrip(b, m.pkg) }
...
func doBuildWdrip(b *BuildContext, from *file.Path) error {
	bcmd := "wdripmac"
	target := filepath.Join(fmt.Sprintf("build/bin/wdrip"))
	switch b.Ctx.Arch {
	case "arm64":
		bcmd = "wdriparm64"
		target = filepath.Join(fmt.Sprintf("build/bin/wdrip.arm64"))
	case "amd64":
		bcmd = "wdriplinux"
		target = filepath.Join(fmt.Sprintf("build/bin/wdrip.amd64"))
	}
	m := cmd.NewCmd(
		"make", bcmd,
	)
	...
	if err := cmd.CmdError(status); err != nil {
		return fmt.Errorf("build wdrip error: %s", err.Error())
	}
...
func EnsureBucket(region string, bucket string) error {
	endpoint := fmt.Sprintf("%s.oss.aliyuncs.com", region)
	m := cmd.NewCmd(
		"ossutil", "--endpoint", endpoint, "ls",
	)
	status := <-m.Start()
	if err := cmd.CmdError(status); err != nil {
		return err
	}
	result := append(status.Stderr, status.Stdout...)
	if strings.Contains(strings.Join(result, ""), bucket) {
		return nil
	}
	m = cmd.NewCmd(
		"ossutil", "--endpoint", endpoint, "mb", fmt.Sprintf("oss://%s", bucket),
	)
	status = <-m.Start()
	return cmd.CmdError(status)
}
