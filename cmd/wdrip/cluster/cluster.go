...
wdrip create \
	--name wdrip-stack-027 \ 
	--cluster-config /Users/aoxn/work/wdrip/pkg/iaas/provider/ros/example/bootcfg.yaml

## Get cluster list
wdrip get --resource cluster
or wdrip get
...
func NewCommand() *cobra.Command {
	...
	//cmd.WdripFlags().StringVar(&flags.ProviderCfgFile, "provider-config-file", "", "provider config file")
	cmd.Flags().StringVar(&flags.Config, "config", "", "cluster boot config")
	//cmd.WdripFlags().StringVar(&flags.Options, "provider", "ros", "cluster name, support ros")
...
func get(flags *v1.WdripOptions) error                             { return iaas.Get(flags, &cmdLine) }
func edit(flags *v1.WdripOptions) error                            { return iaas.Edit(flags, &cmdLine) }
func create(flags *v1.WdripOptions) error                          { return iaas.Create(flags) }
func delete(flags *v1.WdripOptions, cmd *v1.CommandLineArgs) error { return iaas.Delete(flags, cmd) }
func scale(flags *v1.WdripOptions) error {
	return iaas.Scale(flags, flags.ClusterName, flags.TargetCount)
}
func watch(flags *v1.WdripOptions) error { return iaas.WatchResult(flags, flags.ClusterName) }
