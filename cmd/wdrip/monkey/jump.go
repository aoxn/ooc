...
func TriggerBackup(ctx *prvd.Context, client client.Client) {
	...
	idx := index.NewGenericIndexer(spec.Spec.ClusterID, ctx.Provider())
	snap := backup.NewBareSnapshot(idx)
...
func NewInitializedPrvdCtx() (cluster.Cluster, *prvd.Context, error) {
	...
	mctx, err := prvd.NewContext(&v1.WdripOptions{}, &spec.Spec)
	if err != nil {
		return nil, mctx, errors.Wrapf(err, "new prvd context")
	}
	mprvd := mctx.Provider()
	idx := index.NewGenericIndexer(spec.Spec.ClusterID, mctx.Provider())
	id, err := idx.GetCluster(spec.Spec.ClusterID)
	if err != nil {
...
func (d *StopProcess) Execute() (int, error) {
	...
	mfunc := func(i prvd.Instance) error {
		...
		_, err := mprvd.RunCommand(d.PrvdCtx, i.Id, cmd)
		return err
	}
	return d.doOperate(mfunc)
}
