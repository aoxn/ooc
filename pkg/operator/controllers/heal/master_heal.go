...
var _ manager.Runnable = &MasterHeal{}
...
func (m *MasterHeal) Start(ctx mctx.Context) error {
...
func (m *MasterHeal) InjectCache(cache cache.Cache) error {
	m.cache = cache
	return nil
}

func (m *MasterHeal) InjectClient(me client.Client) error {
	m.client = me
	return nil
}

func (m *MasterHeal) String() string {
	return fmt.Sprintf("member heal: %s", m.state)
}

func (m *MasterHeal) Dirty() bool { return m.state == StateDirty }

// mark mark state of member heal center
func (m *MasterHeal) mark(state string) { m.state = state }

func (m *MasterHeal) NotifyScale(result chan struct{}) { m.dispatcher <- &Event{Done: result} }

func (m *MasterHeal) NotifyNodeEvent(result chan struct{}) { m.nodes <- &Event{Done: result} }
...
func (m *MasterHeal) RemoveFollower() (string, error) {
...
//   3. try to reapir
func (m *MasterHeal) DeepCheck() error { return m.doCheck("DeepCheck") }
...
//   4. try to repair
func (m *MasterHeal) ShallowCheck() error { return m.doCheck("Shallow") }

func (m *MasterHeal) doCheck(checkType string) error {
...
func (m *MasterHeal) Synchronization() error {
	// protected by lock.
...
func (m *MasterHeal) dosync() error {
	...
	klog.Infof("begin sync: load scaling group detail(len=%d), "+
		"CRD(%d), MasterNodes(%d) ", len(detail.Instances), len(mCrds), len(mNodes))
...
func (m *MasterHeal) NewMasterCRD(nodes []v1.Node) {
	for _, n := range nodes {
		me := api.Master{
			TypeMeta: metav1.TypeMeta{
				Kind:       "Master",
...
func (m *MasterHeal) ResetNode(
	...
	canAdmitECS := func(
		...
		for _, t := range e.Tags {
			if t.Key == OvmLastUpdate {
				klog.Infof("ovm last re-initialize node at %s", t.Val)
				return h.After(t.Val.(string), duration)
			}
		}
		klog.Infof("tag %s not found, mark date and return", OvmLastUpdate)
		err := m.prvd.TagECS(cctx, e.Id, provider.Value{Key: OvmLastUpdate, Val: h.Now()})
		if err != nil {
	...
	canAdmitNode := func(
		...
		nodes, err := h.MasterNodes(m.client)
		if err != nil {
			if apierrors.IsNotFound(err) {
	...
	for _, e := range ecs {
		...
		err = h.WaitMasterReady(m.client, e.Id)
		if err != nil {
